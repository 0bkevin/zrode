...
- Publishes the CLI package (`apps/server`, npm package `zrode`) with OIDC trusted publishing from the same workflow file:
...
- `ZRODE_WEB_ROUTER_URL`: defaults to `https://app.zrode.codes`.
- `ZRODE_WEB_LATEST_DOMAIN`: defaults to `latest.app.zrode.codes`.
- `ZRODE_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.zrode.codes`.

Required Vercel domains:

- `app.zrode.codes`: the router domain users open, updated by stable releases.
- `latest.app.zrode.codes`: channel alias updated by stable releases.
- `nightly.app.zrode.codes`: channel alias updated by nightly releases.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__zrode/channel?channel=latest` or
`/__zrode/channel?channel=nightly`; the router stores the
`zrode_web_channel` cookie and rewrites future requests on `app.zrode.codes` to
...
through `/__zrode/channel` on the router domain so the user's channel cookie is
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `app.zrode.codes` points at a deployment containing the router
...
- Publishes the CLI package (`apps/server`, npm package `zrode`) to the `nightly` npm dist-tag using the same nightly version.
...
- Repository slug source:
  - `ZRODE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `ZRODE_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `zrode` (or rename package first if needed).
