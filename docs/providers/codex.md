...
This guide is for people who want to use more than one Codex account in Zrode.
...
- both accounts can see the same Zrode/Codex sessions
...
In Zrode Settings, name it something obvious:
...
In Zrode Settings, add another Codex provider:
...
Zrode shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Zrode treats it
