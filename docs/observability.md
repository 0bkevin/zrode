...
Zrode has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.zrode/userdata/logs/server.trace.ndjson`).
...
```bash
export ZRODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export ZRODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export ZRODE_OTLP_SERVICE_NAME=zrode-local
```

Optional:

```bash
export ZRODE_TRACE_MIN_LEVEL=Info
export ZRODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `ZRODE_OTLP_*`.

macOS app bundle example:

```bash
ZRODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
ZRODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
ZRODE_OTLP_SERVICE_NAME=zrode-desktop \
"/Applications/Zrode.app/Contents/MacOS/Zrode"
```
...
```bash
tail -f "$ZRODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$ZRODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$ZRODE_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `zrode-local`, `zrode-dev`, or `zrode-desktop`
...
- `ZRODE_OTLP_TRACES_URL` was not set
...
- `ZRODE_TRACE_FILE`: override trace file path
- `ZRODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `ZRODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `ZRODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `ZRODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `ZRODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `ZRODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `ZRODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `ZRODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `ZRODE_OTLP_SERVICE_NAME`: service name, default `zrode-server`
