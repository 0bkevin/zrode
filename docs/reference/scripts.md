...
- Dev commands default `ZRODE_STATE_DIR` to `~/.t3/dev` to keep dev state isolated from desktop/prod state.
...
- Desktop production windows load the bundled UI from `zrode://app/index.html` (not a `127.0.0.1` document URL).
...
- Signed macOS builds also require `ZRODE_APPLE_TEAM_ID` and
  `ZRODE_MACOS_PROVISIONING_PROFILE`. The passkey RP domain is derived from
  `ZRODE_CLERK_PUBLISHABLE_KEY` unless `ZRODE_CLERK_PASSKEY_RP_DOMAINS` overrides it.
...
Set `ZRODE_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `ZRODE_DEV_INSTANCE`)
- Example: `ZRODE_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `ZRODE_PORT_OFFSET` to a numeric offset.
