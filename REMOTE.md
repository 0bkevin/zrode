...
Use this when you want to connect to a Zrode server from another device such as a phone, tablet, or separate desktop app.
...
- HTTPS/WSS-compatible endpoints work from `https://app.zrode.codes`, but are not made the default
  automatically.
...
If the copied link points directly at `http://192.168.x.y:3773`, open it from a client that can reach that LAN address. If it points at `https://app.zrode.codes/pair?...`, the hosted web app will save the environment and connect directly to the backend URL in the link.
...
with the same server-side behavior as `zrode serve --tailscale-serve`, then the server asks Tailscale
...
For `https://app.zrode.codes`, prefer an HTTPS Tailnet or other HTTPS endpoint. A plain `http://100.x.y.z:3773` endpoint can still work from a desktop client or another browser page served over HTTP, but it will not work from the hosted HTTPS app because of browser mixed-content rules.
...
Run the server with `zrode serve`.

```bash
npx zrode serve --host "$(tailscale ip -4)"
```

`zrode serve` starts the server without opening a browser and prints:
...
Use `zrode serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.

For hosted web pairing over Tailscale HTTPS, opt in to Tailscale Serve:

```bash
npx zrode serve --tailscale-serve
```
...
```bash
npx zrode serve --tailscale-serve --tailscale-serve-port 8443
```

> Note
> The GUIs do not currently support adding projects on remote environments.
> For now, use `zrode project ...` on the server machine instead.
> Full GUI support for remote project management is coming soon.

### Option 3: Desktop-Managed SSH Launch

Use this when you want the desktop app to start or reuse Zrode on another machine over SSH.
...
5. Confirm the launch. The desktop app probes the host, starts or reuses a remote Zrode server, opens a local port forward, and saves the environment.

After setup, the renderer connects to a local forwarded HTTP/WebSocket endpoint. The remote host still owns the actual Zrode server, projects, files, git state, terminals, and provider sessions.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.zrode/ssh-launch/<host-key>/`, starts or reuses a remote Zrode server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. Zrode uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, Zrode first checks whether `node` is already available on `PATH`. If it is missing, the launcher tries common non-interactive shell locations and version-manager shims/activation hooks:
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path Zrode uses:
...
1. `zrode serve` issues a one-time owner pairing token.
...
The hosted web app at `https://app.zrode.codes` can save a remote backend in browser local storage from a URL like:

```text
https://app.zrode.codes/pair?host=https://backend.example.com:3773#token=PAIRCODE
```
...
Hosted pairing does not proxy traffic through Zrode. The browser still connects directly to the backend URL in the pairing link.

## Managing Access Later

Use `zrode auth` to manage access after the initial pairing flow.
...
Use `zrode auth --help` and the nested subcommand help pages for the full reference.
...
- Use `zrode auth` to revoke credentials or sessions you no longer trust.
