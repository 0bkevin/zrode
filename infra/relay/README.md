...
The relay is intentionally not in the hot path for normal Zrode traffic. After a client connects,
regular API and WebSocket traffic goes directly between that client and the selected environment.
See the [T3 Connect architecture overview](../../docs/cloud/zrode-connect-auth-flow.html) for the larger system
design.
...
- Linking Zrode environments to a cloud account.
...
```sh
vp run --filter zrode-relay deploy
```
...
```sh
vp run --filter zrode-relay deploy -- --stage prod
vp run --filter zrode-relay deploy -- --env-file .env.local
```
...
- [T3 Connect Architecture Overview](../../docs/cloud/zrode-connect-auth-flow.html) for the full link,
