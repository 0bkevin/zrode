...
describe("reconcileRootEnvRelayUrl", () => {
  ...
  it("preserves unrelated root env entries while replacing a previous relay URL", () => {
    expect(
      reconcileRootEnvRelayUrl(
        "ZRODE_CLERK_PUBLISHABLE_KEY=pk_test_example\nZRODE_RELAY_URL=https://old.example.test\n",
...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "zrode-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "zrode-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "ZRODE_MOBILE_OTLP_TRACES_DATASET=zrode-mobile-traces-dev",
        "ZRODE_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "ZRODE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "ZRODE_RELAY_CLIENT_OTLP_TRACES_DATASET=zrode-relay-client-traces-dev",
        "ZRODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
    ...
    expect(
      reconcileRootEnvPublicConfig(
        [
          "ZRODE_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "ZRODE_RELAY_URL=https://old.example.test",
          "ZRODE_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "ZRODE_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "ZRODE_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "ZRODE_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "ZRODE_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "ZRODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "ZRODE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "ZRODE_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "ZRODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
