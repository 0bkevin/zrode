...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    ZRODE_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    ZRODE_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    ZRODE_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    ZRODE_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    ZRODE_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    ZRODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("ZRODE_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("ZRODE_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "ZrodeRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Zrode relay through Alchemy."));
