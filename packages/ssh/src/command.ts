...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@zrode/contracts";
...
export function resolveRemoteZrodeCliPackageSpec(input: {
  ...
  if (!input.isDevelopment && PUBLISHABLE_ZRODE_VERSION_PATTERN.test(appVersion)) {
    return `zrode@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "zrode@nightly";
  }

  return input.updateChannel === "nightly" ? "zrode@nightly" : "zrode@latest";
}
