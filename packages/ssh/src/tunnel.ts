import type { DesktopSshEnvironmentBootstrap, DesktopSshEnvironmentTarget } from "@zrode/contracts";
import * as NetService from "@zrode/shared/Net";
import { extractJsonObject, fromLenientJson } from "@zrode/shared/schemaJson";
import { satisfiesSemverRange } from "@zrode/shared/semver";
...
export interface RemoteZrodeRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemoteZrodeRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemoteZrodeRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  ZRODE_NODE_ENGINE_RANGE=@@ZRODE_NODE_ENGINE_RANGE@@
  if [ -z "$ZRODE_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$ZRODE_NODE_ENGINE_RANGE" <<'NODE'
@@ZRODE_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for ZRODE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$ZRODE_NODE_BIN/node" ]; then
        PATH="$ZRODE_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@ZRODE_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
ZRODE_NODE_SCRIPT_PATH=@@ZRODE_NODE_SCRIPT_PATH@@
if [ -n "$ZRODE_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$ZRODE_NODE_SCRIPT_PATH" "$@"
fi
if command -v zrode >/dev/null 2>&1; then
  exec zrode "$@"
fi
if command -v npx >/dev/null 2>&1; then
  exec npx --yes @@ZRODE_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  exec npm exec --yes @@ZRODE_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the zrode CLI and could not install @@ZRODE_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@ZRODE_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.zrode/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.zrode"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-zrode.sh"
RUNNER_NEXT="$STATE_DIR/run-zrode.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@ZRODE_DEFAULT_REMOTE_PORT@@" "@@ZRODE_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@ZRODE_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@ZRODE_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@ZRODE_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@ZRODE_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@ZRODE_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@ZRODE_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env ZRODE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@ZRODE_READY_TIMEOUT_MS@@"; then
    printf 'Remote Zrode server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.zrode/ssh-launch/@@ZRODE_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.zrode"
RUNNER_FILE="$STATE_DIR/run-zrode.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteZrodeRunnerScript(input?: RemoteZrodeRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "zrode@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      ZRODE_PACKAGE_SPEC: packageSpec,
      ZRODE_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      ZRODE_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

function buildRemoteNodeEnvScript(input?: RemoteZrodeRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      ZRODE_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      ZRODE_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemoteZrodeRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    ZRODE_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    ZRODE_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteZrodeRunnerScript(input)),
    ZRODE_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    ZRODE_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    ZRODE_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    ZRODE_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    ZRODE_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    ZRODE_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    ZRODE_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteZrodeRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    ZRODE_STATE_KEY: remoteStateKey(target),
    ZRODE_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteZrodeRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteZrodeRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteZrodeRunnerOptions;
...
>()("@zrode/ssh/SshEnvironmentManager") {
