...
>()("zrode/diagnostics/ProcessDiagnostics") {}
...
function assertDescendantPid(
  ...
  if (pid === process.pid) {
    return Effect.fail(toProcessDiagnosticsError("Refusing to signal the Zrode server process."));
  }
      ...
      return descendant
        ...
        : Effect.fail(
            toProcessDiagnosticsError(
              `Process ${pid} is not a live descendant of the Zrode server.`,
            ),
          );
    }),
  );
