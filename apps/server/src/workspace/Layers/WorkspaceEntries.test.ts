...
const TestLayer = Layer.empty.pipe(
  ...
  Layer.provide(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "zrode-workspace-entries-test-",
    }),
  ),
...
it.layer(TestLayer)("WorkspaceEntriesLive", (it) => {
  ...
  describe("search", () => {
    ...
    it.effect("deduplicates concurrent index builds for the same cwd", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "zrode-workspace-concurrent-build-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
    ...
    it.effect("limits concurrent directory reads while walking the filesystem", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "zrode-workspace-read-concurrency-" });
