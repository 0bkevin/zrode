  ...
  Effect.mapError(
    () =>
      new EnvironmentHttpInternalServerError({
        message: "ZRODE_RELAY_URL must be configured as a secure absolute HTTPS origin.",
      }),
  ),
);
...
const relayClientRequest = <A>(
  ...
  HttpClientRequest.post(input.url).pipe(
    ...
    Effect.mapError(
      (cause) =>
        new EnvironmentHttpInternalServerError({
          message: `Zrode Connect relay request failed: ${String(cause)}`,
        }),
    ),
...
const reconcileDesiredCloudLinkWith = Effect.fn("environment.cloud.reconcileDesiredLinkWith")(
  ...
  Effect.catchIf(
    ServerSecretStore.isSecretStoreError,
    failEnvironmentCloudInternalError("Could not persist desired Zrode Connect link state."),
  ),
  Effect.catchTags({
