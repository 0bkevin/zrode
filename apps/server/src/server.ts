...
import { ProjectSessionHistoryImportReactorLive } from "./orchestration/Layers/ProjectSessionHistoryImportReactor.ts";
...
import { ProviderSessionHistoryImporterLive } from "./provider/Layers/ProviderSessionHistoryImporter.ts";
...
// Zrode's primary transport is long-lived WebSocket RPC, whose Effect scope finalizer
...
const ReactorLayerLive = Layer.empty.pipe(
  ...
  Layer.provideMerge(ProjectSessionHistoryImportReactorLive),
  Layer.provideMerge(ProviderSessionHistoryImporterLive),
...
const WorkspaceFileEventsLayerLive = WorkspaceFileEvents.layer.pipe(
  Layer.provide(WorkspacePaths.layer),
);
...
const RuntimeDependenciesLive = RuntimeCoreDependenciesLive.pipe(
  ...
  Layer.provideMerge(ProviderUsageHistory.layer.pipe(Layer.provide(PersistenceLayerLive))),
      ...
      Effect.gen(function* () {
        ...
        yield* Effect.forkScoped(
          Effect.sleep("250 millis").pipe(
            ...
            Effect.tap(() => Effect.logInfo("Zrode Connect desired link reconciled on startup")),
            Effect.catch((cause) =>
              Effect.logWarning("Failed to reconcile Zrode Connect desired link on startup", {
                cause,
              }),
            ),
          ),
