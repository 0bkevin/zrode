...
import { GitCommandError, type SourceControlProviderError } from "@zrode/contracts";
...
const CLONE_URLS = {
  ...
  url: "https://github.com/octocat/zrode",
  sshUrl: "git@github.com:octocat/zrode.git",
};
...
function makeLayer(input: {
  ...
  return SourceControlRepositoryService.layer.pipe(
    ...
    Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "zrode-source-control-repos-" })),
...
it.effect("looks up repositories through the requested provider without search", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(calls, [{ cwd: "/workspace", repository: "octocat/zrode" }]);
...
it.effect("clones a looked-up repository into the requested destination", () =>
  Effect.gen(function* () {
    ...
    const parent = yield* fs.makeTempDirectoryScoped({
      prefix: "zrode-source-control-clone-parent-",
    });
    const destinationPath = `${parent}/zrode`;
    ...
    yield* Effect.gen(function* () {
      ...
      assert.deepStrictEqual(cloneCalls, [
        {
          cwd: parent,
          args: ["clone", CLONE_URLS.url, "zrode"],
        },
      ]);
    }).pipe(
      Effect.provide(
...
it.effect("publishes by creating the repository, adding a remote, and pushing upstream", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(createCalls, [
      { cwd: "/workspace", repository: "octocat/zrode", visibility: "private" },
    ]);
