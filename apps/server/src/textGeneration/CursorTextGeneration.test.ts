...
import { CursorSettings, ProviderInstanceId } from "@zrode/contracts";
...
const CursorTextGenerationTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "zrode-cursor-text-generation-test-",
...
function withFakeAcpAgent<A, E, R>(
  ...
  return Effect.gen(function* () {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "zrode-cursor-text-acp-"));
...
it.layer(CursorTextGenerationTestLayer)("CursorTextGeneration", (it) => {
  it.effect("uses ACP model config options instead of raw CLI model ids", () => {
    const requestLogDir = mkdtempSync(path.join(os.tmpdir(), "zrode-cursor-text-log-"));
    const requestLogPath = path.join(requestLogDir, "requests.ndjson");

    return withFakeAcpAgent(
      {
        ZRODE_ACP_REQUEST_LOG_PATH: requestLogPath,
        ZRODE_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
  ...
  it.effect("closes the ACP child process after text generation completes", () => {
    const exitLogDir = mkdtempSync(path.join(os.tmpdir(), "zrode-cursor-text-exit-log-"));
