...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:ZrodeOrg/zrode.git"]);
      ...
      expect(identity?.canonicalKey).toBe("github.com/zrodeorg/zrode");
      expect(normalizeResolvedPath(resolvedIdentityRoot)).toBe(normalizeResolvedPath(resolvedCwd));
      expect(identity?.displayName).toBe("zrodeorg/zrode");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("zrodeorg");
  ...
  it.effect("returns the git top-level root path when resolving from a nested workspace", () =>
    Effect.gen(function* () {
      ...
      yield* git(repoRoot, ["remote", "add", "origin", "git@github.com:ZrodeOrg/zrode.git"]);
  ...
  it.effect("prefers upstream over origin when both remotes are configured", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["remote", "add", "upstream", "git@github.com:ZrodeOrg/zrode.git"]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@gitlab.com:ZrodeOrg/platform/zrode.git"]);

      const resolver = yield* RepositoryIdentityResolver.RepositoryIdentityResolver;
      const identity = yield* resolver.resolve(cwd);

      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/zrodeorg/platform/zrode");
      expect(identity?.displayName).toBe("zrodeorg/platform/zrode");
      expect(identity?.owner).toBe("zrodeorg");
      expect(identity?.name).toBe("zrode");
    }).pipe(Effect.provide(RepositoryIdentityResolver.layer)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/zrodeorg/zrode");
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/zrodeorg/zrode");

      yield* git(cwd, ["remote", "set-url", "origin", "git@github.com:ZrodeOrg/zrode-next.git"]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/zrodeorg/zrode");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/zrodeorg/zrode-next");
      expect(refreshedIdentity?.displayName).toBe("zrodeorg/zrode-next");
