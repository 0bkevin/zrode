...
} from "@zrode/shared/git";
import {
  getChangeRequestTerminologyForKind,
  type ChangeRequestTerminology,
} from "@zrode/shared/sourceControl";

import { GitManagerError } from "@zrode/contracts";
import { TextGeneration } from "../textGeneration/TextGeneration.ts";
import { ProjectSetupScriptRunner } from "../project/Services/ProjectSetupScriptRunner.ts";
import { extractBranchNameFromRemoteRef } from "./remoteRefs.ts";
import { ServerSettingsService } from "../serverSettings.ts";
import type { GitManagerServiceError } from "@zrode/contracts";
import { GitVcsDriver, type GitStatusDetails } from "../vcs/GitVcsDriver.ts";
import { SourceControlProviderRegistry } from "../sourceControl/SourceControlProviderRegistry.ts";
import type { ChangeRequest } from "@zrode/contracts";
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `zrode/pr-${pullRequest.number}/${suffix}`;
}
...
export const makeGitManager = Effect.fn("makeGitManager")(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(tempDir, `zrode-pr-body-${process.pid}-${randomUUID()}.md`);
    yield* fileSystem
