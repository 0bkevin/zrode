...
layer("ProviderGrokTokenIdentity migration", (it) => {
  it.effect("invalidates Grok rows and all provider scan cursors", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      yield* runMigrations({ toMigrationInclusive: 40 });
      yield* sql`
        INSERT INTO provider_token_entries (provider, entry_key, sampled_epoch_ms, tokens)
        VALUES ('grok', 'unified:1', 1, 1), ('claude', 'claude-entry', 1, 1)
      `;
      yield* sql`
        INSERT INTO provider_token_files (path, mtime_ms, size_bytes)
        VALUES ('unified.jsonl', 1, 1)
      `;
      ...
      assert.deepStrictEqual(executed, [[41, "ProviderGrokTokenIdentity"]]);
