...
const configuredRelayUrl = repoEnv.VITE_ZRODE_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.ZRODE_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: ZRODE_BUNDLED_DEV=1 pnpm dev:web
const bundledDevEnv = process.env.ZRODE_BUNDLED_DEV?.trim().toLowerCase();
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_ZRODE_RELAY_URL": JSON.stringify(configuredRelayUrl),
