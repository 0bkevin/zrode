import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@zrode/contracts";
...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.zrode/worktrees/zrode-mvp/zrode-4e609bb8",
    );
    expect(result).toBe("zrode-4e609bb8");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.zrode\\worktrees\\zrode-mvp\\zrode-4e609bb8",
    );
    expect(result).toBe("zrode-4e609bb8");
  });

  it("uses the final segment even when outside ~/.zrode/worktrees", () => {
