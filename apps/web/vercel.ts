...
const ROUTER_HOST = "app.zrode.codes";
const HOSTED_WEB_CHANNEL_COOKIE = "zrode_web_channel";
const LATEST_ORIGIN = "https://latest.app.zrode.codes";
const NIGHTLY_ORIGIN = "https://nightly.app.zrode.codes";
...
export const config: VercelConfig = {
  buildCommand:
    'turbo build --filter @zrode/web && bun ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "bun add -g turbo && bun install --filter '@zrode/contracts' --filter '@zrode/client-runtime' --filter '@zrode/scripts' --filter '@zrode/web'",
  routes: [
    {
      src: "/__zrode/channel",
