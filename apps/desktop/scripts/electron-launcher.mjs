// This file mostly exists because we want dev mode to say "Zrode (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Zrode (Dev)" : "Zrode (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.t3tools.zrode.dev.${devBundleIdSuffix || "local"}`
  : "com.t3tools.zrode";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["zrode-dev"] : ["zrode"];
...
const developmentMacIconPngPath = NodePath.join(
  ...
  "assets",
  "dev",
  "zrode-dev-macos-1024.png",
);
// oxlint-disable-next-line zrode/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["ZRODE_PORT", environment.ZRODE_PORT],
    ["ZRODE_HOME", environment.ZRODE_HOME],
    ["ZRODE_COMMIT_HASH", environment.ZRODE_COMMIT_HASH],
    ["ZRODE_OTLP_TRACES_URL", environment.ZRODE_OTLP_TRACES_URL],
    ["ZRODE_OTLP_EXPORT_INTERVAL_MS", environment.ZRODE_OTLP_EXPORT_INTERVAL_MS],
    ["ZRODE_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --zrode-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
