    ...
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          ...
          ZRODE_DEV_REMOTE_T3_SERVER_ENTRY_PATH: " /remote/server.mjs ",
      ...
      assert.equal(environment.appUserModelId, "com.t3tools.zrode.dev");
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          ZRODE_DESKTOP_APP_USER_MODEL_ID: " com.t3tools.zrode.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.t3tools.zrode.dev.local");
    }),
  );
