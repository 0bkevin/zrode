...
const AppPackageMetadata = Schema.Struct({
  zrodeCommitHash: Schema.optional(Schema.String),
});
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.zrodeCommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),
