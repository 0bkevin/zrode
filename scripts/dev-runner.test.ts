  ...
  describe("resolveOffset", () => {
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid ZRODE_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults ZRODE_HOME to ~/.zrode when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.ZRODE_HOME, path.resolve(NodeOS.homedir(), ".zrode"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.ZRODE_HOME, path.resolve("/tmp/custom-zrode"));
    ...
    it.effect("uses custom zrodeHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
        ...
        assert.equal(env.ZRODE_HOME, path.resolve("/tmp/my-zrode"));
      }),
    );
