...
export const DEFAULT_ZRODE_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".zrode"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@zrode/contracts",
    "--filter=@zrode/web",
    "--filter=zrode",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=zrode"],
  "dev:web": ["run", "dev", "--filter=@zrode/web"],
  "dev:desktop": ["run", "dev", "--filter=@zrode/desktop", "--filter=@zrode/web", "--parallel"],
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid ZRODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric ZRODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed ZRODE_DEV_INSTANCE=${seed}` };
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly zrodeHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(zrodeHome);
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read ZRODE_PORT_OFFSET/ZRODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  zrodeHome: Flag.string("home-dir").pipe(
