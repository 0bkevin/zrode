...
- Dev commands default `ZRODE_STATE_DIR` to `~/.zrode/dev` to keep dev state isolated from desktop/prod state.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --base-dir ~/.zrode-2`
...
- Desktop packaging includes `apps/server/dist` (the `zrode` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
