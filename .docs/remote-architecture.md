...
This document describes the target architecture for first-class remote environments in Zrode.
...
- Keep the Zrode server as the execution boundary.
...
Zrode already has a clean runtime boundary: the client talks to a Zrode server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one Zrode server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Zrode server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the Zrode runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running Zrode server instance.
...
How does the client speak WebSocket to a Zrode server?
...
- best fit for hosted or self-managed remote Zrode deployments
...
For Zrode, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
Tailscale-backed access sits here architecturally even though the current implementation is endpoint discovery rather than a Zrode-managed tunnel. It contributes private-network endpoints and lets the existing HTTP/WebSocket client path do the actual connection.
...
- reach a machine
- probe it
- launch or reuse a remote Zrode server
...
How does a Zrode server come to exist on the target machine?
...
The user or operator already runs Zrode on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in Zrode:
...
The recommended Zrode flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies Zrode availability.
3. Desktop launches or reuses a remote Zrode server.
...
This is the inverse of remote launch: a local Zrode server is already running, and the client publishes it through a tunnel.
...
- A manually hosted Zrode server might be reached through direct `wss`.
...
Zrode already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
Zrode should not copy that part.

Zrode already has the right runtime boundary:

- one Zrode server per environment
- ordinary HTTP/WebSocket between client and environment

So Zrode should borrow Zed's launch discipline, not its transport protocol.
